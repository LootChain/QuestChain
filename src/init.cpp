...
public:
    ...
    // QuestChain Specific WalletInitInterface InitQuestChainSendSettings
 ...
 * between questchaind, and questchain-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("questchain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"QuestChain Core - \""));
    ...
    strUsage += HelpMessageOpt("-sporkaddr=<questchainaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/questchainpay/questchain>";
    const std::string URL_WEBSITE = "<https://questchain.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("questchain-loadblk");
...
/** Sanity checks
 *  Ensure that QuestChain Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("QuestChain Core version %s\n", FormatFullVersion());
}
...
{
    // Make sure only a single QuestChain Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if QuestChain Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if QuestChain Core is started while in a temporary directory.\n",
        ...
        do {
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/questchainpay/questchain/pull/1817 and https://github.com/questchainpay/questchain/pull/1743
    ...
    // ********************************************************* Step 10b: setup QuestChainSend

    g_wallet_init_interface->InitQuestChainSendSettings();
    CQuestChainSend::InitStandardDenominations();
    ...
    // ********************************************************* Step 10c: schedule QuestChain-specific tasks
    ...
    if (fMasternodeMode) {
        scheduler.scheduleEvery(boost::bind(&CQuestChainSendServer::DoMaintenance, boost::ref(questchainSendServer), boost::ref(*g_connman)), 1 * 1000);
    }
