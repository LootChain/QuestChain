...
private:
     ...
     */
    uint256 nQuestChainSendSalt;

    /**
     * Fetches QuestChainSend salt from database or generates and saves a new one if no salt was found in the db
     */
    void InitQuestChainSendSalt();
...
public:
    ...
    // get the QuestChainSend chain depth for a given input
    int GetRealOutpointQuestChainSendRounds(const COutPoint& outpoint, int nRounds = 0) const;
    // respect current settings
    int GetCappedOutpointQuestChainSendRounds(const COutPoint& outpoint) const;
