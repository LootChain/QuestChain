...
{
    for (const auto& d : CQuestChainSend::GetStandardDenominations()) {
...

    {
        ...
        for (MasterKeyMap::value_type& pMasterKey : mapMasterKeys)
        {
            ...
            if (CCryptoKeyStore::Unlock(_vMasterKey))
            {
                ...
                if (pMasterKey.second.nDeriveIterations < 1000000)
                    pMasterKey.second.nDeriveIterations = 1000000;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
    ...
    CCrypter crypter;
    int64_t nStartTime = GetTimeMillis();
    crypter.SetKeyFromPassphrase(strWalletPassphrase, kMasterKey.vchSalt, 1000000, kMasterKey.nDerivationMethod);
    ...
    if (kMasterKey.nDeriveIterations < 1000000)
        kMasterKey.nDeriveIterations = 1000000;
...
// Recursively determine the rounds of a given input (How deep is the QuestChainSend chain for a given input)
int CWallet::GetRealOutpointQuestChainSendRounds(const COutPoint& outpoint, int nRounds) const
{
    LOCK(cs_wallet);

    const int nRoundsMax = MAX_PRIVCYSEND_ROUNDS + questchainSendClient.nQuestChainSendRandomRounds;
    ...
    if (CQuestChainSend::IsCollateralAmount(txOutRef->nValue)) {
        *nRoundsRef = -3;
    ...
    if (!CQuestChainSend::IsDenominatedAmount(txOutRef->nValue)) { //NOT DENOM
        *nRoundsRef = -2;
        LogPrint(BCLog::PRIVCYSEND, "%s UPDATED   %-70s %3d\n", __func__, outpoint.ToStringShort(), *nRoundsRef);
        return *nRoundsRef;
    }
    ...
    for (const auto& txinNext : wtx->tx->vin) {
        if (IsMine(txinNext)) {
            int n = GetRealOutpointQuestChainSendRounds(txinNext.prevout, nRounds + 1);
...
int CWallet::GetCappedOutpointQuestChainSendRounds(const COutPoint& outpoint) const
{
    LOCK(cs_wallet);
    int realQuestChainSendRounds = GetRealOutpointQuestChainSendRounds(outpoint);
    return realQuestChainSendRounds > questchainSendClient.nQuestChainSendRounds ? questchainSendClient.nQuestChainSendRounds : realQuestChainSendRounds;
}
...
bool CWallet::IsDenominated(const COutPoint& outpoint) const
{
    ...
    return CQuestChainSend::IsDenominatedAmount(it->second.tx->vout[outpoint.n].nValue);
}

bool CWallet::IsFullyMixed(const COutPoint& outpoint) const
{
    int nRounds = GetRealOutpointQuestChainSendRounds(outpoint);
    // Mix again if we don't have N rounds yet
    if (nRounds < questchainSendClient.nQuestChainSendRounds) return false;
    ...
    if (nRounds < questchainSendClient.nQuestChainSendRounds + questchainSendClient.nQuestChainSendRandomRounds) {
        CDataStream ss(SER_GETHASH, PROTOCOL_VERSION);
        ss << outpoint << nQuestChainSendSalt;
        uint256 nHash;
...
CAmount CWalletTx::GetAnonymizedCredit(const CCoinControl* coinControl) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++)
    {
        ...
        if (pwallet->IsSpent(hashTx, i) || !CQuestChainSend::IsDenominatedAmount(txout.nValue)) continue;
...
{
    if(!questchainSendClient.fEnableQuestChainSend) return 0;
    ...
    const CAmount nSmallestDenom = CQuestChainSend::GetSmallestDenomination();
    const CAmount nMixingCollateral = CQuestChainSend::GetCollateralAmount();
    for (const auto& item : vecTally) {
        bool fIsDenominated = CQuestChainSend::IsDenominatedAmount(item.nAmount);
...
float CWallet::GetAverageAnonymizedRounds() const
{
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        nTotal += GetCappedOutpointQuestChainSendRounds(outpoint);
        nCount++;
    }
...
CAmount CWallet::GetNormalizedAnonymizedBalance() const
{
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        if (!CQuestChainSend::IsDenominatedAmount(nValue)) continue;
        if (it->second.GetDepthInMainChain() < 0) continue;

        int nRounds = GetCappedOutpointQuestChainSendRounds(outpoint);
        nTotal += nValue * nRounds / questchainSendClient.nQuestChainSendRounds;
    }
        ...
        for (auto pcoin : GetSpendableTXs()) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (nCoinType == CoinType::ONLY_FULLY_MIXED) {
                    if (!CQuestChainSend::IsDenominatedAmount(pcoin->tx->vout[i].nValue)) continue;
                    ...
                    if (CQuestChainSend::IsCollateralAmount(pcoin->tx->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !CQuestChainSend::IsDenominatedAmount(pcoin->tx->vout[i].nValue);
                    ...
                    found = CQuestChainSend::IsCollateralAmount(pcoin->tx->vout[i].nValue);
                } else {
                    found = true;
                }
                if(!found) continue;
...
void CWallet::InitQuestChainSendSalt()
{
    // Avoid fetching it multiple times
    assert(nQuestChainSendSalt.IsNull());

    WalletBatch batch(*database);
    batch.ReadQuestChainSendSalt(nQuestChainSendSalt);

    while (nQuestChainSendSalt.IsNull()) {
        // We never generated/saved it
        nQuestChainSendSalt = GetRandHash();
        batch.WriteQuestChainSendSalt(nQuestChainSendSalt);
    }
}
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    for (const auto& d : CQuestChainSend::GetStandardDenominations()) // loop through predefined denoms
    {
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, CoinType nCoinType) const
{
    ...
    if (nCoinType == CoinType::ONLY_FULLY_MIXED) {
        ...
        // try not to use denominated coins when not needed, save denoms for questchainsend
    ...
    for (unsigned int tryDenom = tryDenomStart; tryDenom < 2; tryDenom++)
    {
        ...
        for (const COutput &output : vCoins)
        {
            ...
            if (tryDenom == 0 && CQuestChainSend::IsDenominatedAmount(coin.txout.nValue)) continue; // we don't want denom values on first run
...
bool CWallet::SelectPSInOutPairsByDenominations(int nDenom, CAmount nValueMax, std::vector< std::pair<CTxDSIn, CTxOut> >& vecPSInOutPairsRet)
{
    ...
    if (!CQuestChainSend::IsValidDenomination(nDenom)) {
        return false;
    }
    CAmount nDenomAmount = CQuestChainSend::DenominationToAmount(nDenom);
    ...
    for (const auto& out : vCoins) {
        ...
        int nRounds = GetRealOutpointQuestChainSendRounds(txin.prevout);
...
bool CWallet::SelectCoinsGroupedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed, int nMaxOupointsPerAddress) const
{
    ...
    CAmount nSmallestDenom = CQuestChainSend::GetSmallestDenomination();
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fSkipDenominated && CQuestChainSend::IsDenominatedAmount(wtx.tx->vout[i].nValue)) continue;

            if(fAnonymizable) {
                // ignore collaterals
                if(CQuestChainSend::IsCollateralAmount(wtx.tx->vout[i].nValue)) continue;
...
bool CWallet::SelectDenominatedAmounts(CAmount nValueMax, std::set<CAmount>& setAmountsRet) const
{
    ...
    return nValueTotal >= CQuestChainSend::GetSmallestDenomination();
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    if (!GetCollateralTxDSIn(txdsinCollateral, nValue)) {
        strReason = "QuestChainSend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    // CQuestChainSend::IsCollateralAmount in GetCollateralTxDSIn should already take care of this
    if (nValue >= CQuestChainSend::GetCollateralAmount() * 2) {
        ...
        reservekey.KeepKey();
        // return change
        txCollateral.vout.push_back(CTxOut(nValue - CQuestChainSend::GetCollateralAmount(), scriptChange));
    } else { // nValue < CQuestChainSend::GetCollateralAmount() * 2
                ...
                if (pick_new_inputs) {
                    ...
                    if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoinsTmp, nValueIn, &coin_control)) {
                        if (coin_control.nCoinType == CoinType::ONLY_NONDENOMINATED) {
                            strFailReason = _("Unable to locate enough QuestChainSend non-denominated funds for this transaction.");
                        } else if (coin_control.nCoinType == CoinType::ONLY_FULLY_MIXED) {
                            strFailReason = _("Unable to locate enough QuestChainSend denominated funds for this transaction.");
                            strFailReason += " " + _("QuestChainSend uses exact denominated amounts to send funds, you might simply need to mix some more coins.");
