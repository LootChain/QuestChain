...
void CoinControlDialog::buttonToggleLockClicked()
{
    ...
    if(ui->radioListMode->isChecked()){
        ...
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++){
            ...
            // Don't toggle the lock state of partially mixed coins if they are not hidden in QuestChainSend mode
            if (coinControl()->IsUsingQuestChainSend() && !fHideAdditional && !model->isFullyMixed(outpt)) {
                continue;
            }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            // QuestChainSend Fee = overpay
            if(coinControl()->IsUsingQuestChainSend() && nChange > 0)
            {
    ...
    int nDisplayUnit = BitcoinUnits::QCH;
...
void CoinControlDialog::useQuestChainSend(bool fUseQuestChainSend)
{
    CoinControlDialog::mode = fUseQuestChainSend ? CoinControlDialog::Mode::PRIVCYSEND : CoinControlDialog::Mode::NORMAL;
}
...
CCoinControl* CoinControlDialog::coinControl()
{
    if (CoinControlDialog::mode == CoinControlDialog::Mode::NORMAL) {
        ...
        coinControlNormal.UseQuestChainSend(false);
        return &coinControlNormal;
    } else {
        static CCoinControl coinControlQuestChainSend;
        coinControlQuestChainSend.UseQuestChainSend(true);
        return &coinControlQuestChainSend;
    }
}
...
void CoinControlDialog::updateView()
{
    ...
    case Mode::NORMAL:
        if (fHideAdditional) {
            ...
            strHideButton = tr("Hide QuestChainSend coins");
        }
        break;
    case Mode::PRIVCYSEND:
        if (fHideAdditional) {
            strHideButton = tr("Show all QuestChainSend coins");
        } else {
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            bool fQuestChainSendAmount = CQuestChainSend::IsDenominatedAmount(nAmount) || CQuestChainSend::IsCollateralAmount(nAmount);

            if (coinControl()->IsUsingQuestChainSend()) {
                fFullyMixed = model->isFullyMixed(outpoint);
                if ((fHideAdditional && !fFullyMixed) || (!fHideAdditional && !fQuestChainSendAmount)) {
                    coinControl()->UnSelect(outpoint);
                    continue;
                }
            } else {
                if (fHideAdditional && fQuestChainSendAmount) {
                    coinControl()->UnSelect(outpoint);
                    continue;
                }
            }
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show questchain address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // QuestChainSend rounds
            int nRounds = model->getRealOutpointQuestChainSendRounds(outpoint);
            ...
            if (coinControl()->IsUsingQuestChainSend() && !fHideAdditional && !fFullyMixed) {
                itemOutput->setDisabled(true);
            }
        }
