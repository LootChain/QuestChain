    ...
    transactionView(0), abandonAction(0), columnResizingFixer(0)
{
    ...
    typeWidget->addItem("QuestChainSend", TransactionFilterProxy::TYPE(TransactionRecord::QuestChainSend));
    typeWidget->addItem(tr("QuestChainSend Make Collateral Inputs"), TransactionFilterProxy::TYPE(TransactionRecord::QuestChainSendMakeCollaterals));
    typeWidget->addItem(tr("QuestChainSend Create Denominations"), TransactionFilterProxy::TYPE(TransactionRecord::QuestChainSendCreateDenominations));
    typeWidget->addItem(tr("QuestChainSend Denominate"), TransactionFilterProxy::TYPE(TransactionRecord::QuestChainSendDenominate));
    typeWidget->addItem(tr("QuestChainSend Collateral Payment"), TransactionFilterProxy::TYPE(TransactionRecord::QuestChainSendCollateralPayment));
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if(_model)
    {
        ...
        if (_model->getOptionsModel())
        {
            ...
            connect(_model->getOptionsModel(), SIGNAL(questchainSendEnabledChanged()), this, SLOT(updateQuestChainSendVisibility()));
        }
...
void TransactionView::showAddressQRCode()
{
    ...
    dialog->setInfo(tr("QR code"), "questchain:"+strAddress, "", strAddress);
    dialog->show();
}
...
void TransactionView::updateQuestChainSendVisibility()
{
    bool fEnabled = questchainSendClient.fEnableQuestChainSend;
    // If QuestChainSend gets enabled use "All" else "Most common"
    typeWidget->setCurrentIndex(fEnabled ? 0 : 1);
    // Hide all QuestChainSend related filters
