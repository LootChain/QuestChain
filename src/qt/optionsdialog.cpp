    ...
    mapper(0)
{
    ...
    /* Remove Wallet/QuestChainSend tabs in case of -disablewallet */
    if (!enableWallet) {
        ui->stackedWidgetOptions->removeWidget(ui->pageWallet);
        ui->btnWallet->hide();
        ui->stackedWidgetOptions->removeWidget(ui->pageQuestChainSend);
        ui->btnQuestChainSend->hide();
    } else {
        pageButtons.addButton(ui->btnWallet, pageButtons.buttons().size());
        pageButtons.addButton(ui->btnQuestChainSend, pageButtons.buttons().size());
    }
    ...
    // Store the current QuestChainSend enabled state to recover it if it gets changed but the dialog gets not accepted but declined.
#ifdef ENABLE_WALLET
    fQuestChainSendEnabledPrev = questchainSendClient.fEnableQuestChainSend;
    connect(this, &OptionsDialog::rejected, [=]() {
        if (fQuestChainSendEnabledPrev != questchainSendClient.fEnableQuestChainSend) {
            ui->questchainSendEnabled->click();
        }
    });
#endif
}
...
void OptionsDialog::setModel(OptionsModel *_model)
{
...
#ifdef ENABLE_WALLET
        // If -enablequestchainsend was passed in on the command line, set the checkbox
        // to the value given via commandline and disable it (make it unclickable).
        if (strLabel.contains("-enablequestchainsend")) {
            bool fEnabled = questchainSendClient.fEnableQuestChainSend;
            ui->questchainSendEnabled->setChecked(fEnabled);
            ui->questchainSendEnabled->setEnabled(false);
        }
#endif
    ...
    connect(ui->questchainSendEnabled, &QCheckBox::clicked, [=](bool fChecked) {
#ifdef ENABLE_WALLET
        questchainSendClient.fEnableQuestChainSend = fChecked;
#endif
        updateQuestChainSendVisibility();
        if (_model != nullptr) {
            _model->emitQuestChainSendEnabledChanged();
        }
        updateWidth();
    });
}
...
void OptionsDialog::setMapper()
{
    ...
    mapper->addMapping(ui->questchainSendEnabled, OptionsModel::QuestChainSendEnabled);

    /* Wallet */
    mapper->addMapping(ui->coinControlFeatures, OptionsModel::CoinControlFeatures);
    mapper->addMapping(ui->showMasternodesTab, OptionsModel::ShowMasternodesTab);
    mapper->addMapping(ui->showAdvancedPSUI, OptionsModel::ShowAdvancedPSUI);
    mapper->addMapping(ui->showQuestChainSendPopups, OptionsModel::ShowQuestChainSendPopups);
    mapper->addMapping(ui->lowKeysWarning, OptionsModel::LowKeysWarning);
    mapper->addMapping(ui->questchainSendMultiSession, OptionsModel::QuestChainSendMultiSession);
    mapper->addMapping(ui->spendZeroConfChange, OptionsModel::SpendZeroConfChange);
    mapper->addMapping(ui->questchainSendRounds, OptionsModel::QuestChainSendRounds);
    mapper->addMapping(ui->questchainSendAmount, OptionsModel::QuestChainSendAmount);
...
void OptionsDialog::updateQuestChainSendVisibility()
{
#ifdef ENABLE_WALLET
    bool fEnabled = questchainSendClient.fEnableQuestChainSend;
#else
    bool fEnabled = false;
#endif
    ui->btnQuestChainSend->setVisible(fEnabled);
}
