...
SendCoinsDialog::SendCoinsDialog(bool _fQuestChainSend, QWidget* parent) :
    QDialog(parent),
    ...
    fFeeMinimized(true),
    fQuestChainSend(_fQuestChainSend)
{
    ui->setupUi(this);
    ...
    if (settings.contains("bUseDarkSend")) {
        settings.setValue("bUseQuestChainSend", settings.value("bUseDarkSend").toBool());
        settings.remove("bUseDarkSend");
    }
    if (!settings.contains("bUseQuestChainSend"))
        settings.setValue("bUseQuestChainSend", false);
    ...
    if (fQuestChainSend) {
        ui->sendButton->setText("QuestChainS&end");
        ui->sendButton->setToolTip(tr("Confirm the QuestChainSend action"));
    } else {
...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients)
{
    ...
    ctrl.UseQuestChainSend(fQuestChainSend);
    ...
    if(ctrl.IsUsingQuestChainSend()) {
        questionString.append(tr("using") + " <b>" + tr("QuestChainSend funds only") + "</b>");
    } else {
    ...
    if(txFee > 0)
    {
        ...
        if (ctrl.IsUsingQuestChainSend()) {
            questionString.append(" " + tr("(QuestChainSend transactions have higher fees usually due to no change output being allowed)"));
        }
    }
    ...
    if (ctrl.IsUsingQuestChainSend()) {
        ...
        if (nInputs >= 10 && ctrl.IsUsingQuestChainSend()) {
            questionString.append("<br />");
            questionString.append("<span style='" + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_ERROR) + "'>");
            questionString.append(tr("Warning: Using QuestChainSend with %1 or more inputs can harm your privacy and is not recommended").arg(10));
            questionString.append("</span> ");
        }
    }
...
void SendCoinsDialog::showEvent(QShowEvent* event)
{
    ...
    if (!event->spontaneous()) {
        CoinControlDialog::useQuestChainSend(fQuestChainSend);
    }
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid QuestChain address"));
        }
        else // Valid address
        {
