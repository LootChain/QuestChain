    ...
    walletModel(0)
{
    ...
    questchainSendCoinsPage = new SendCoinsDialog(true);
    ...
    addWidget(questchainSendCoinsPage);
    ...
    connect(questchainSendCoinsPage, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));
...
void WalletView::setClientModel(ClientModel *_clientModel)
{
    ...
    questchainSendCoinsPage->setClientModel(_clientModel);
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WalletView::setWalletModel(WalletModel *_walletModel)
{
    ...
    questchainSendCoinsPage->setModel(_walletModel);
...
void WalletView::processNewTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    QSettings settings;
    if (!settings.value("fShowQuestChainSendPopups").toBool()) {
        QVariant nType = ttm->data(index, TransactionTableModel::TypeRole);
        if (nType == TransactionRecord::QuestChainSendDenominate ||
            nType == TransactionRecord::QuestChainSendCollateralPayment ||
            nType == TransactionRecord::QuestChainSendMakeCollaterals ||
            nType == TransactionRecord::QuestChainSendCreateDenominations) return;
    }
...
void WalletView::gotoQuestChainSendCoinsPage(QString addr)
{
    setCurrentWidget(questchainSendCoinsPage);

    if (!addr.isEmpty())
        questchainSendCoinsPage->setAddress(addr);
}
