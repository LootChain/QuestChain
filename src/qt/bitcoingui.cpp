...
BitcoinGUI::BitcoinGUI(const NetworkStyle *networkStyle, QWidget *parent) :
    ...
    quitAction(0),
    sendCoinsAction(0),
    questchainSendCoinsAction(0),
    sendCoinsMenuAction(0),
    questchainSendCoinsMenuAction(0),
    ...
    showQuestChainSendHelpAction(0),
    trayIcon(0),
    trayIconMenu(0),
    dockIconMenu(0),
    notificator(0),
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a QuestChain address"));
    ...
    questchainSendCoinsAction = new QToolButton(this);
    questchainSendCoinsAction->setText("&QuestChainSend");
    questchainSendCoinsAction->setStatusTip(tr("QuestChainSend coins to a QuestChain address"));
    questchainSendCoinsAction->setToolTip(questchainSendCoinsAction->statusTip());
    questchainSendCoinsAction->setCheckable(true);
    tabGroup->addButton(questchainSendCoinsAction);

    questchainSendCoinsMenuAction = new QAction(questchainSendCoinsAction->text(), this);
    questchainSendCoinsMenuAction->setStatusTip(questchainSendCoinsAction->statusTip());
    questchainSendCoinsMenuAction->setToolTip(questchainSendCoinsMenuAction->statusTip());

    receiveCoinsAction = new QToolButton(this);
    receiveCoinsAction->setText(tr("&Receive"));
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and questchain: URIs)"));
    ...
    connect(questchainSendCoinsAction, SIGNAL(clicked()), this, SLOT(showNormalIfMinimized()));
    connect(questchainSendCoinsAction, SIGNAL(clicked()), this, SLOT(gotoQuestChainSendCoinsPage()));
    connect(questchainSendCoinsMenuAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(questchainSendCoinsMenuAction, SIGNAL(triggered()), this, SLOT(gotoQuestChainSendCoinsPage()));
    ...
    aboutAction->setStatusTip(tr("Show information about QuestChain Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your QuestChain addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified QuestChain addresses"));
    ...
    openAction->setStatusTip(tr("Open a questchain: URI or payment request"));

    showHelpMessageAction = new QAction(tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible QuestChain command-line options").arg(tr(PACKAGE_NAME)));

    showQuestChainSendHelpAction = new QAction(tr("&QuestChainSend information"), this);
    showQuestChainSendHelpAction->setMenuRole(QAction::NoRole);
    showQuestChainSendHelpAction->setStatusTip(tr("Show the QuestChainSend basic information"));
    ...
    connect(showQuestChainSendHelpAction, SIGNAL(triggered()), this, SLOT(showQuestChainSendHelpClicked()));
...
void BitcoinGUI::createMenuBar()
{
    ...
    help->addAction(showQuestChainSendHelpAction);
    help->addSeparator();
...
void BitcoinGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        ...
        questchainSendCoinsAction->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
        ...
        toolbar->addWidget(questchainSendCoinsAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    if(_clientModel)
    {
        ...
        if(optionsModel)
        {
            ...
            connect(optionsModel, SIGNAL(questchainSendEnabledChanged()), this, SLOT(updateQuestChainSendVisibility()));
        }
    } else {
    ...
    updateQuestChainSendVisibility();
}
...
#ifdef ENABLE_WALLET
    questchainSendCoinsAction->setEnabled(enabled && questchainSendClient.fEnableQuestChainSend);
    questchainSendCoinsMenuAction->setEnabled(enabled && questchainSendClient.fEnableQuestChainSend);
#else
    questchainSendCoinsAction->setEnabled(enabled);
    questchainSendCoinsMenuAction->setEnabled(enabled);
#endif // ENABLE_WALLET
...
void BitcoinGUI::createIconMenu(QMenu *pmenu)
{
    ...
    pmenu->addAction(questchainSendCoinsMenuAction);
...
void BitcoinGUI::showQuestChainSendHelpClicked()
{
    if(!clientModel)
        return;
...
void BitcoinGUI::gotoQuestChainSendCoinsPage(QString addr)
{
    questchainSendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoQuestChainSendCoinsPage(addr);
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to QuestChain network", "", count));
    } else {
...
void BitcoinGUI::updateQuestChainSendVisibility()
{
#ifdef ENABLE_WALLET
    bool fEnabled = questchainSendClient.fEnableQuestChainSend;
#else
    bool fEnabled = false;
#endif
    // QuestChainSend button is the third QToolButton, show/hide the underlying QAction
    ...
    questchainSendCoinsMenuAction->setVisible(fEnabled);
    showQuestChainSendHelpAction->setVisible(fEnabled);
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the questchain logo
...
#ifdef ENABLE_WALLET
    disableAppNap |= questchainSendClient.fQuestChainSendRunning;
#endif // ENABLE_WALLET
    if (disableAppNap) {
...
{
    QString strTitle = tr("QuestChain Core"); // default title
    ...
    // Append title to "QuestChain Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
