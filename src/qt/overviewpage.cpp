...
public:
    explicit TxViewDelegate(QObject* parent = nullptr) :
        QAbstractItemDelegate(), unit(BitcoinUnits::QCH)
    {
    ...
    txdelegate(new TxViewDelegate(this))
{
    ...
    ui->labelQuestChainSendSyncStatus->setText("(" + tr("out of sync") + ")");
    ui->labelTransactionsStatus->setText("(" + tr("out of sync") + ")");

    // hide PS frame (helps to preserve saved size)
    // we'll setup and make it visible in questchainSendStatus() later
    ui->frameQuestChainSend->setVisible(false);

    // start with displaying the "out of sync" warnings
    showOutOfSyncWarning(true);

    // Disable questchainSendClient builtin support for automatic backups while we are in GUI,
    // we'll handle automatic backups and user warnings in questchainSendStatus()
    questchainSendClient.fCreateAutoBackups = false;

    timer = new QTimer(this);
    connect(timer, SIGNAL(timeout()), this, SLOT(questchainSendStatus()));
}
...
{
    if(timer) disconnect(timer, SIGNAL(timeout()), this, SLOT(questchainSendStatus()));
    delete ui;
}
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("QCH")
        updateDisplayUnit();
        ...
        connect(model->getOptionsModel(), SIGNAL(questchainSendRoundsChanged()), this, SLOT(updateQuestChainSendProgress()));
        connect(model->getOptionsModel(), SIGNAL(privateSentAmountChanged()), this, SLOT(updateQuestChainSendProgress()));
        connect(model->getOptionsModel(), SIGNAL(advancedPSUIChanged(bool)), this, SLOT(updateAdvancedPSUI(bool)));
        connect(model->getOptionsModel(), &OptionsModel::questchainSendEnabledChanged, [=]() {
            questchainSendStatus(true);
        });

        connect(ui->toggleQuestChainSend, SIGNAL(clicked()), this, SLOT(toggleQuestChainSend()));

        // questchainsend buttons will not react to spacebar must be clicked on
        ui->toggleQuestChainSend->setFocusPolicy(Qt::NoFocus);
    }
}
...
void OverviewPage::showOutOfSyncWarning(bool fShow)
{
    ...
    ui->labelQuestChainSendSyncStatus->setVisible(fShow);
    ui->labelTransactionsStatus->setVisible(fShow);
}

void OverviewPage::updateQuestChainSendProgress()
{
    ...
    QString strQuestChainSendAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, questchainSendClient.nQuestChainSendAmount * COIN, false, BitcoinUnits::separatorAlways);

    if(currentBalance == 0)
    {
        ui->questchainSendProgress->setValue(0);
        ui->questchainSendProgress->setToolTip(tr("No inputs detected"));

        // when balance is zero just show info from settings
        strQuestChainSendAmount = strQuestChainSendAmount.remove(strQuestChainSendAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strQuestChainSendAmount + " / " + tr("%n Rounds", "", questchainSendClient.nQuestChainSendRounds);
    ...
    if(nMaxToAnonymize > questchainSendClient.nQuestChainSendAmount*COIN) nMaxToAnonymize = questchainSendClient.nQuestChainSendAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= questchainSendClient.nQuestChainSendAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to mix %1")
                                          .arg(strQuestChainSendAmount));
        strQuestChainSendAmount = strQuestChainSendAmount.remove(strQuestChainSendAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strQuestChainSendAmount + " / " + tr("%n Rounds", "", questchainSendClient.nQuestChainSendRounds);
    } else {
        ...
        strAmountAndRounds = "<span style='" + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_ERROR) + "'>" +
                ...
                " / " + tr("%n Rounds", "", questchainSendClient.nQuestChainSendRounds) + "</span>";
    }
    ui->labelAmountRounds->setText(strAmountAndRounds);
    ...
    float denomWeight = 1;
    float anonNormWeight = questchainSendClient.nQuestChainSendRounds;
    ...
    ui->questchainSendProgress->setValue(progress);

    QString strToolPip = ("<b>" + tr("Overall progress") + ": %1%</b><br/>" +
                          tr("Denominated") + ": %2%<br/>" +
                          tr("Partially mixed") + ": %3%<br/>" +
                          tr("Mixed") + ": %4%<br/>" +
                          tr("Denominated inputs have %5 of %n rounds on average", "", questchainSendClient.nQuestChainSendRounds))
            .arg(progress).arg(denomPart).arg(anonNormPart).arg(anonFullPart)
            .arg(nAverageAnonymizedRounds);
    ui->questchainSendProgress->setToolTip(strToolPip);
}
...
void OverviewPage::questchainSendStatus(bool fForce)
{
    ...
    if (fMasternodeMode || nWalletBackups <= 0) {
        DisableQuestChainSendCompletely();
        if (nWalletBackups <= 0) {
            ui->labelQuestChainSendEnabled->setToolTip(tr("Automatic backups are disabled, no mixing available!"));
        }
        return;
    }

    bool fIsEnabled = questchainSendClient.fEnableQuestChainSend;
    ui->frameQuestChainSend->setVisible(fIsEnabled);
    if (!fIsEnabled) {
    ...
    // Wrap all questchainsend related widgets we want to show/hide state based.
    // Value of the map contains a flag if this widget belongs to the advanced
    // QuestChainSend UI option or not. True if it does, false if not.
    std::map<QWidget*, bool> questchainSendWidgets = {
        {ui->labelCompletitionText, true},
        {ui->questchainSendProgress, true},
    ...
    auto setWidgetsVisible = [&](bool fVisible) {
        ...
        for (const auto& it : questchainSendWidgets) {
    ...
    if(nBestHeight > questchainSendClient.nCachedNumBlocks && GetTime() - nLastDSProgressBlockTime <= 1) return;
    ...
    ui->labelQuestChainSendEnabled->setToolTip(strKeysLeftText);

    if (!questchainSendClient.fQuestChainSendRunning) {
        if (nBestHeight != questchainSendClient.nCachedNumBlocks) {
            questchainSendClient.nCachedNumBlocks = nBestHeight;
            updateQuestChainSendProgress();
        }

        setWidgetsVisible(false);
        ui->toggleQuestChainSend->setText(tr("Start Mixing"));

        QString strEnabled = tr("Disabled");
        // Show how many keys left in advanced PS UI mode only
        if (fShowAdvancedPSUI) strEnabled += ", " + strKeysLeftText;
        ui->labelQuestChainSendEnabled->setText(strEnabled);
    ...
    if (nWalletBackups > 0 && walletModel->getKeysLeftSinceAutoBackup() < PRIVCYSEND_KEYS_THRESHOLD_WARNING) {
        ...
        if(settings.value("fLowKeysWarning").toBool()) {
            ...
            ui->labelQuestChainSendEnabled->setToolTip(strWarn);
            LogPrint(BCLog::PRIVCYSEND, "OverviewPage::questchainSendStatus -- Very low number of keys left since last automatic backup, warning user and trying to create new backup...\n");
            QMessageBox::warning(this, "QuestChainSend", strWarn, QMessageBox::Ok, QMessageBox::Ok);
        } else {
            LogPrint(BCLog::PRIVCYSEND, "OverviewPage::questchainSendStatus -- Very low number of keys left since last automatic backup, skipping warning and trying to create new backup...\n");
        }
        ...
        if(!walletModel->autoBackupWallet(strBackupWarning, strBackupError)) {
            if (!strBackupWarning.isEmpty()) {
                ...
                LogPrint(BCLog::PRIVCYSEND, "OverviewPage::questchainSendStatus -- WARNING! Something went wrong on automatic backup: %s\n", strBackupWarning.toStdString());

                QMessageBox::warning(this, "QuestChainSend",
            ...
            if (!strBackupError.isEmpty()) {
                ...
                LogPrint(BCLog::PRIVCYSEND, "OverviewPage::questchainSendStatus -- ERROR! Failed to create automatic backup: %s\n", strBackupError.toStdString());
    ...
    QString strEnabled = questchainSendClient.fQuestChainSendRunning ? tr("Enabled") : tr("Disabled");
    // Show how many keys left in advanced PS UI mode only
    if(fShowAdvancedPSUI) strEnabled += ", " + strKeysLeftText;
    ui->labelQuestChainSendEnabled->setText(strEnabled);
    ...
    if(nWalletBackups == -1) {
        ...
        QString strError =  tr("ERROR! Failed to create automatic backup") + ", " +
                            ...
                            tr("Mixing is disabled, please close your wallet and fix the issue!");
        ui->labelQuestChainSendEnabled->setToolTip(strError);
        ...
        ui->labelQuestChainSendEnabled->setToolTip(strWarning);
    }

    // check questchainsend status and unlock if needed
    if(nBestHeight != questchainSendClient.nCachedNumBlocks) {
    ...
    ui->labelSubmittedDenom->setText(QString(questchainSendClient.GetSessionDenoms().c_str()));
}

void OverviewPage::toggleQuestChainSend(){
    QSettings settings;
    // Popup some information on first mixing
    QString hasMixed = settings.value("hasMixed").toString();
    if(hasMixed.isEmpty()){
        QMessageBox::information(this, "QuestChainSend",
                tr("If you don't want to see internal QuestChainSend fees/transactions select \"Most Common\" as Type on the \"Transactions\" tab."),
                QMessageBox::Ok, QMessageBox::Ok);
        settings.setValue("hasMixed", "hasMixed");
    }
    if(!questchainSendClient.fQuestChainSendRunning){
        const CAmount nMinAmount = CQuestChainSend::GetSmallestDenomination() + CQuestChainSend::GetMaxCollateralAmount();
        if(currentBalance < nMinAmount){
            QString strMinAmount(BitcoinUnits::formatWithUnit(nDisplayUnit, nMinAmount));
            QMessageBox::warning(this, "QuestChainSend",
                tr("QuestChainSend requires at least %1 to use.").arg(strMinAmount),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        ...
        if (walletModel && walletModel->getEncryptionStatus() == WalletModel::Locked)
        {
            ...
            if(!ctx.isValid())
            {
                ...
                questchainSendClient.nCachedNumBlocks = std::numeric_limits<int>::max();
                QMessageBox::warning(this, "QuestChainSend",
                    tr("Wallet is locked and user declined to unlock. Disabling QuestChainSend."),
                    QMessageBox::Ok, QMessageBox::Ok);
                LogPrint(BCLog::PRIVCYSEND, "OverviewPage::toggleQuestChainSend -- Wallet is locked and user declined to unlock. Disabling QuestChainSend.\n");
                return;
            }
        }
    ...
    questchainSendClient.fQuestChainSendRunning = !questchainSendClient.fQuestChainSendRunning;
    questchainSendClient.nCachedNumBlocks = std::numeric_limits<int>::max();
    ...
    if(!questchainSendClient.fQuestChainSendRunning){
        ...
        ui->toggleQuestChainSend->setText(tr("Stop Mixing"));
    }
}
...
void OverviewPage::DisableQuestChainSendCompletely() {
    ui->toggleQuestChainSend->setText("(" + tr("Disabled") + ")");
    ui->frameQuestChainSend->setEnabled(false);
    if (nWalletBackups <= 0) {
        ui->labelQuestChainSendEnabled->setText("<span style='" + GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_ERROR) + "'>(" + tr("Disabled") + ")</span>");
    }
