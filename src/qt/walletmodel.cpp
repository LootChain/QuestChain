...
void WalletModel::pollBalanceChanged()
{
    ...
    if(fForceCheckBalanceChanged || chainActive.Height() != cachedNumBlocks || questchainSendClient.nQuestChainSendRounds != cachedQuestChainSendRounds)
    {
        fForceCheckBalanceChanged = false;

        // Balance and number of transactions might have changed
        cachedNumBlocks = chainActive.Height();
        cachedQuestChainSendRounds = questchainSendClient.nQuestChainSendRounds;
...
int WalletModel::getRealOutpointQuestChainSendRounds(const COutPoint& outpoint) const
{
    return wallet->GetRealOutpointQuestChainSendRounds(outpoint);
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered questchain address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        for (const SendCoinsRecipient &rcp : recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal questchain:URI (questchain:XyZ...?message=example)
            {
