...
class CQuestChainSendStatusUpdate
{
public:
    int nSessionID;
    PoolState nState;
    ...
    CQuestChainSendStatusUpdate(int nSessionID, PoolState nState, int nEntriesCount, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID) :
...
class CQuestChainSendAccept
{
public:
    int nDenom;
    CMutableTransaction txCollateral;

    CQuestChainSendAccept() :
        nDenom(0),
        txCollateral(CMutableTransaction()){};

    CQuestChainSendAccept(int nDenom, const CMutableTransaction& txCollateral) :
        nDenom(nDenom),
    ...
    friend bool operator==(const CQuestChainSendAccept& a, const CQuestChainSendAccept& b)
    {
...
public:
    ...
    CQuestChainSendEntry(const std::vector<CTxDSIn>& vecTxDSIn, const std::vector<CTxOut>& vecTxOut, const CTransaction& txCollateral) :
            vecTxDSIn(vecTxDSIn),
            vecTxOut(vecTxOut),
...
public:
    ...
    CQuestChainSendQueue(int nDenom, COutPoint outpoint, int64_t nTime, bool fReady) :
        nDenom(nDenom),
    ...
    friend bool operator==(const CQuestChainSendQueue& a, const CQuestChainSendQueue& b)
    {
 ...
 */
class CQuestChainSendBroadcastTx
{
private:
    // memory only
...
public:
    ...
    CQuestChainSendBroadcastTx(const CTransactionRef& _tx, COutPoint _outpoint, int64_t _sigTime) :
        nConfirmedHeight(-1),
        tx(_tx),
    ...
    friend bool operator==(const CQuestChainSendBroadcastTx& a, const CQuestChainSendBroadcastTx& b)
    {
        return *a.tx == *b.tx;
    }
    friend bool operator!=(const CQuestChainSendBroadcastTx& a, const CQuestChainSendBroadcastTx& b)
    ...
    {
        return *this != CQuestChainSendBroadcastTx();
    }
...
// base class
class CQuestChainSendBaseSession
{
protected:
    mutable CCriticalSection cs_questchainsend;

    std::vector<CQuestChainSendEntry> vecEntries; // Masternode/clients entries
...
// base class
class CQuestChainSendBaseManager
{
protected:
    mutable CCriticalSection cs_vecqueue;

    // The current mixing sessions in progress on the network
    std::vector<CQuestChainSendQueue> vecQuestChainSendQueue;

    void SetNull();
    void CheckQueue();

public:
    CQuestChainSendBaseManager() :
        vecQuestChainSendQueue() {}

    int GetQueueSize() const { return vecQuestChainSendQueue.size(); }
    bool GetQueueItemAndTry(CQuestChainSendQueue& dsqRet);
};
...
private:
    ...
    CQuestChainSend() {}
    ~CQuestChainSend() {}
    CQuestChainSend(CQuestChainSend const&) = delete;
    CQuestChainSend& operator=(CQuestChainSend const&) = delete;

    // static members
    static std::vector<CAmount> vecStandardDenominations;
    static std::map<uint256, CQuestChainSendBroadcastTx> mapDSTX;
...
public:
    ...
    static void AddDSTX(const CQuestChainSendBroadcastTx& dstx);
    static CQuestChainSendBroadcastTx GetDSTX(const uint256& hash);
