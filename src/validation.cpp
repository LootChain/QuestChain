...
#if defined(NDEBUG)
# error "QuestChain Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in QuestChain
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
  ...
  int nHeight = nPrevHeight + 1;
      ...
      if (nHeight ==1) { nSubsidy = 80000000 * COIN;
      } else if (nHeight > 1       && nHeight <= 40000)   { nSubsidy = 2000 * COIN;
      } else if (nHeight > 40000   && nHeight <= 80000)  { nSubsidy = 1000 * COIN;
      } else if (nHeight > 80000  && nHeight <= 120000)  { nSubsidy = 500 * COIN;
      } else if (nHeight > 120000  && nHeight <= 160000)  { nSubsidy = 250 * COIN;
      } else if (nHeight > 160000  && nHeight <= 200000)  { nSubsidy = 125 * COIN;
      } else if (nHeight > 200000  && nHeight <= 240000)  { nSubsidy = 62.5 * COIN;
      } else if (nHeight > 240000  && nHeight <= 280000)  { nSubsidy = 31.25 * COIN;
      } else if (nHeight > 280000  && nHeight <= 320000)  { nSubsidy = 15.625 * COIN;
      } else if (nHeight > 320000 ) { nSubsidy = 7.8125 * COIN;}
...
void ThreadScriptCheck() {
    RenameThread("questchain-scriptch");
...
static int64_t nTimeQuestChainSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // QCH : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(QCH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(QCH): spork is off, skipping transaction locking checks\n");
    }
    ...
    // QCH : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(QCH): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(QCH): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(QCH): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeQuestChainSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - QuestChain specific: %.2fms [%.2fs (%.2fms/blk)]\n", MICRO * (nTime5 - nTime4), nTimeQuestChainSpecific * MICRO, nTimeQuestChainSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(QCH): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
