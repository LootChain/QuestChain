 ...
 *  We have 4 times smaller block times in QuestChain, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_LEGACY_TXLOCK_REQUEST: // we treat legacy IX messages as TX messages
        {
            ...
            return (!fIgnoreRecentRejects && recentRejects->contains(inv.hash)) ||
                   (inv.type == MSG_DSTX && static_cast<bool>(CQuestChainSend::GetDSTX(inv.hash))) ||
    ...
    /*
        QuestChain Related Inventory Messages
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && it->IsKnownType()) {
            ...
            if (inv.type == MSG_TX || inv.type == MSG_DSTX) {
                CQuestChainSendBroadcastTx dstx;
                if (inv.type == MSG_DSTX) {
                    dstx = CQuestChainSend::GetDSTX(inv.hash);
                }
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (pfrom->nVersion >= SENDDSQUEUE_PROTO_VERSION) {
            // Tell our peer that he should send us QuestChainSend queue messages
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SENDDSQUEUE, true));
        } else {
            // older nodes do not support SENDDSQUEUE and expect us to always send QuestChainSend queue messages
...
#ifdef ENABLE_WALLET
        questchainSendClient.ProcessMessage(pfrom, strCommand, vRecv, *connman);
#endif // ENABLE_WALLET
        questchainSendServer.ProcessMessage(pfrom, strCommand, vRecv, *connman);
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (fSendTrickle && pto->fSendMempool) {
                ...
                for (const auto& txinfo : vtxinfo) {
                    ...
                    int nInvType = MSG_TX;
                    if (CQuestChainSend::GetDSTX(hash)) {
                        nInvType = MSG_DSTX;
                    }
        ...
        // QCH this code also handles non-TXs (QuestChain specific messages)
