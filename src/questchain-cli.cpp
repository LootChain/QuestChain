...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", _("Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to questchaind)"));
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  questchain-cli [options] <command> [params]  " + strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                  "  questchain-cli [options] -named <command> [name=value] ... " + strprintf(_("Send command to %s (with named arguments)"), _(PACKAGE_NAME)) + "\n" +
                  "  questchain-cli [options] help                " + _("List commands") + "\n" +
                  "  questchain-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue &batch_in) override
    {
        ...
        if (!batch[ID_WALLETINFO].isNull()) {
            result.pushKV("walletversion", batch[ID_WALLETINFO]["result"]["walletversion"]);
            result.pushKV("balance", batch[ID_WALLETINFO]["result"]["balance"]);
            result.pushKV("questchainsend_balance", batch[ID_WALLETINFO]["result"]["questchainsend_balance"]);
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to questchain-cli command line.";
                        }
                    }
                } else {
                    // Result
