...
const char * const BITCOIN_CONF_FILENAME = "questchain.conf";
const char * const BITCOIN_PID_FILENAME = "questchaind.pid";
...
const CLogCategoryDesc LogCategories[] =
{
    ...
    {BCLog::MNSYNC, "mnsync"},
    {BCLog::PRIVCYSEND, "questchainsend"},
    {BCLog::SPORK, "spork"},
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "questchaind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QuestChainCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QuestChainCore
    // Mac: ~/Library/Application Support/QuestChainCore
    // Unix: ~/.questchaincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QuestChainCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QuestChainCore";
#else
    // Unix
    return pathRet / ".questchaincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty questchain.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure QuestChain Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("QuestChain Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The QuestChain Core developers";
    }
