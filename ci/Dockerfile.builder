...
# questchain_hash
RUN git clone https://github.com/questchainpay/questchain_hash
RUN cd questchain_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} questchain
RUN useradd -u ${USER_ID} -g questchain -s /bin/bash -m -d /questchain questchain
...
RUN mkdir /questchain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /questchain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /questchain-src
