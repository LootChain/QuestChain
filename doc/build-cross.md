Cross-compiliation of QuestChain Core
===============================

QuestChain Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building QuestChain.
...
When building QuestChain Core, use
...
Note that for WSL the QuestChain Core source path MUST be somewhere in the default mount file system, for
example /usr/src/questchain, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the QuestChain Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the QuestChain Core source code).
