...
*Setup instructions for a Gitian build of QuestChain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the QuestChain
...
- [Building QuestChain Core](#building-questchain-core)
...
Clone the git repositories for QuestChain Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/questchainpay/questchain
...
in the QuestChain Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building QuestChain Core
----------------

To build QuestChain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the QuestChain Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/questchain/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/questchainpay/questchain
...
For example:
```bash
URL=https://github.com/crowning-/questchain.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit questchain=${COMMIT} --url questchain=${URL} ../questchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit questchain=${COMMIT} --url questchain=${URL} ../questchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit questchain=${COMMIT} --url questchain=${URL} ../questchain/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the questchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../questchain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/questchain
SIGPATH=/some/root/path/questchain-detached-sigs

./bin/gbuild --url questchain=${BTCPATH},signature=${SIGPATH} ../questchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/questchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/questchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/questchain-osx-build.assert
```
...
[questchain/gitian.sigs](https://github.com/questchainpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
